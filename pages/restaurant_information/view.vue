            ...
            <uni-forms :modelValue="form" v-if="is_view()">
              ...
              >
                <uni-data-select
                  ...
                  v-if="
                    user_group === '管理员' ||
                    (form['restaurant_information_id'] &&
                      $check_field('set', 'merchant_users')) ||
                    (!form['restaurant_information_id'] &&
                      $check_field('add', 'merchant_users'))
                  "
              ...
              <uni-forms-item
                v-if="
                  $check_field('get', 'restaurant_image') ||
                  $check_field('add', 'restaurant_image') ||
                  $check_field('set', 'restaurant_image')
                "
                label="餐厅图片"
              ...
              >
                ...
                >
                  <image
                    v-if="disabledObj['restaurant_image_isDisabled']"
                    :src="$fullUrl(form['restaurant_image'])"
                  />
                  <image
                    v-if="!disabledObj['restaurant_image_isDisabled']"
                    :src="$fullUrl(form['restaurant_image'])"
                    @click="change_img('restaurant_image')"
                  />
                </view>
                <!-- 添加权限 -->
                <view
                ...
                <view
                  class="diy_field diy_img"
                  v-else-if="$check_field('get', 'restaurant_image')"
                >
                  <image :src="$fullUrl(form['restaurant_image'])" />
                </view>
              </uni-forms-item>
              <uni-forms-item
                v-if="
                  $check_field('get', 'restaurant_type') ||
                  $check_field('add', 'restaurant_type') ||
                  $check_field('set', 'restaurant_type')
                "
                label="餐厅类型"
                ...
                <uni-easyinput
                  ...
                  v-if="
                    ...
                    (!form['restaurant_information_id'] &&
                      $check_field('add', 'restaurant_type'))
                  "
                  :disabled="disabledObj['restaurant_type_isDisabled']"
                />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get', 'restaurant_type')">
              ...
              <uni-forms-item
                v-if="
                  $check_field('get', 'restaurant_details') ||
                  $check_field('add', 'restaurant_details') ||
                  $check_field('set', 'restaurant_details')
                "
                label="餐厅详情"
              ...
              >
                <uni-easyinput
                  type="textarea"
                  v-model="form['restaurant_details']"
                  v-if="
                    user_group === '管理员' ||
                    (form['restaurant_information_id'] &&
                      $check_field('set', 'restaurant_details')) ||
                    (!form['restaurant_information_id'] &&
                      $check_field('add', 'restaurant_details'))
                  "
                  :disabled="disabledObj['restaurant_details_isDisabled']"
                />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get', 'restaurant_details')">
                  {{ form['restaurant_details'] }}
                </text>
              </uni-forms-item>
            </uni-forms>
            <view
              class="form_button"
              v-if="
                $check_action('/restaurant_information/view', 'set') ||
                $check_action('/restaurant_information/view', 'add') ||
                $check_option('/restaurant_information/table', 'examine')
              "
            >
              <button
                size="mini"
...
请求参数
     ...
     */
    submit_check(param) {
      let restaurant_phone_number_phone_regular =
        /^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[0-9]|19[0-35-9])\d{8}$/
      if (
        param.restaurant_phone_number &&
        !restaurant_phone_number_phone_regular.test(
          param.restaurant_phone_number
        )
      ) {
        return '手机号格式错误'
    ...
    successChoose(filePath, type) {
      uni.uploadFile({
        url: getApp().globalData.host + '/api/restaurant_information/upload', //仅为示例，非真实的接口地址
        filePath,
        name: 'file',
    ...
    close_(type) {
      ...
      if (type == 'restaurant_details')
        this['restaurant_details'] = this.form['restaurant_details'] = ''
    },
    submit_() {
      ...
      if (this['restaurant_details'] !== null)
        this.form['restaurant_details'] = this['restaurant_details']
