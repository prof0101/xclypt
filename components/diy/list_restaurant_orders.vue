<template>
  <view
    class="list_restaurant_orders list_com--new"
    style="background-color: #fff"
  >
    <!-- 视图 -->
    <view class="restaurant_orders_block box_wrap">
      <navigator
        class="item_restaurant_orders box_style"
        v-for="(o, i) in list"
        :key="i"
        :url="
          '/pages/restaurant_orders/details?restaurant_orders_id=' +
          o['restaurant_orders_id']
        "
      >
        <view
          class="view"
        ...
        <view
          ...
          v-if="
            0 &&
            $check_index_field('get', 'user_name', '/restaurant_orders/list')
          "
        >
          <view class="title" v-if="true">
            <span>用户姓名</span>
        ...
        <view
          ...
          v-if="
            0 &&
            $check_index_field('get', 'dinner_time', '/restaurant_orders/list')
          "
        >
          <view class="title" v-if="true">
            <span>用餐时间</span>
          ...
          <view class="diy_field time">
            <span>{{ $toTime(o['dinner_time'], 'yyyy-MM-dd hh:mm:ss') }}</span>
          </view>
        </view>
        <view
          class="view"
...
id
     */
    to_nav(id) {
      this.$nav('/pages/restaurant_orders/details?restaurant_orders_id=' + id)
    },
    /**
     * 获取普通用户用户列表
     */
...
.list_restaurant_orders .list_restaurant_orders_table {
  width: 100%;
  padding: 5px 0;
}

.list_restaurant_orders .list_restaurant_orders_table .btn_change_table {
  margin-left: auto;
  font-weight: bold;
  padding: 0.5rem 0;
  width: 100px;
...
.list_restaurant_orders
  .list_restaurant_orders_table
  .restaurant_orders_table_block
  .uni-table-td {
  padding: 8px;
}
...
.list_restaurant_orders .item_restaurant_orders {
  display: flex;
  justify-content: space-between;
  align-items: stretch;
  padding: 0.75rem 1rem;
}
.list_restaurant_orders .item_restaurant_orders + .item_restaurant_orders {
  border-top: 1px solid #dbdbdb;
}
.list_restaurant_orders .item_restaurant_orders .left > image {
  position: relative;
  top: 50%;
...
.list_restaurant_orders .item_restaurant_orders .right_block {
  width: calc(100% - 5rem);
  display: flex;
...
.list_restaurant_orders .top {
  font-size: 0.9rem;
}

.list_restaurant_orders .time {
  font-size: 0.6rem;
  color: var(--color_grey);
}

.list_restaurant_orders .bottom {
  display: flex;
  justify-content: space-between;
